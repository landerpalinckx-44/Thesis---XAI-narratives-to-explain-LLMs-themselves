...
# CLI arguments
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Evaluate narratives using LLM-as-a-Judge")
...
def query_judge(model: str, prompt: str, temperature: float) -> int | None:
    ...
    try:
        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "You are a strict evaluator. You must respond with a single integer between 1 and 5. No explanation, no reasoning, no other text."},
...
def evaluate_entry(entry: dict, temperature: float, persona: str, persona_description: str, shap_table: str) -> dict:
    system_prompt   = entry.get("system_prompt", "")
    original_prompt = entry["prompt"]
    model_choice    = entry["baseline_response"]
    narrative       = entry["story"]

    full_prompt = f"System: {system_prompt}\n\nUser: {original_prompt}" if system_prompt else original_prompt
    ...
    for aspect in config.JUDGE_ASPECTS:
        ...
        if aspect == "faithfulness":
            judge_prompt = builder(persona_description, full_prompt, model_choice, shap_table, narrative)
        else:
        ...
        all_final_scores = [
            v["final_score"] for k, v in aspect_results.items()
...
def build_persona_descriptions(entry: dict) -> dict:
    ...
    profile = {}
    for line in entry["patient_profile"].strip().split("\n"):
        if ":" in line:
...
for i, log_idx in enumerate(indices_to_evaluate, start=1):
    ...
    pkl_path = os.path.join(config.RESULTS_DIR, f"tokenshap_results_instance_{entry['instance_index']}.pkl")
    ...
    for persona, persona_description in persona_descriptions.items():
        ...
        entry["evaluations"][persona] = evaluate_entry(
            entry, args.temperature, persona, persona_description, shap_table
        )
