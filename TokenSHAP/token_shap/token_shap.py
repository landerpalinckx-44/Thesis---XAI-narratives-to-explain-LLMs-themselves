...
class TokenSHAP(BaseSHAP):
    ...
    def _prepare_generate_args(self, content: str, **kwargs) -> Dict:
        ...
        Passes system_prompt through as a keyword argument if provided,
        so the frozen instructions are never part of the perturbed content.
        """
        args = {"prompt": content}
        if "system_prompt" in kwargs:
            args["system_prompt"] = kwargs["system_prompt"]
        return args
    ...
    def _prepare_combination_args(self, combination: List[str], original_content: str) -> Dict:
        ...
        Joins only the (masked) user tokens — the system_prompt is attached
        separately in analyze() via _prepare_generate_args, so it never
        gets perturbed or dropped.
        """
        args = {"prompt": self.splitter.join(combination)}
        if hasattr(self, "_current_system_prompt") and self._current_system_prompt:
            args["system_prompt"] = self._current_system_prompt
        return args
    ...
    ### Updated version of print_colored_text to center colors on zero and preserve sign #########################################################################
    def print_colored_text(self):
        """Print text with tokens colored by importance (zero-centered)"""
        if not hasattr(self, 'shapley_values'):
            raise ValueError("Must run analyze() before visualization")

        # 1. Find the maximum absolute value
        max_abs = max(abs(v) for v in self.shapley_values.values())
        ...
        def get_color(value):
            ...
            if norm_value < 0.5:
                # Negative values scale from Dark Blue (0.0) to White (0.49)
                r = int(255 * (norm_value * 2))
                g = int(255 * (norm_value * 2))
                b = 255
            else:
                # Positive values scale from White (0.5) to Dark Red (1.0)
                r = 255
                g = int(255 * (2 - norm_value * 2))
                b = int(255 * (2 - norm_value * 2))
        ...
        for token, value in self.shapley_values.items():
            color = get_color(value)
            # Print using ANSI escape codes for 24-bit RGB colors
            print(
                f"\033[38;2;{int(color[1:3], 16)};"
                f"{int(color[3:5], 16)};"
                f"{int(color[5:7], 16)}m"
    ...
    ## End of Updated version of print_colored_text to center colors on zero and preserve sign #########################################################################
    ...
    #### Updated version of plot_colored_text to center colors on zero and preserve sign ##############################################################################
    def plot_colored_text(self, new_line: bool = False):
        """
        Plot text visualization with importance colors centered on zero.
        ...
        # 1. Calculate the maximum absolute value to center our scale on 0
        max_abs = max(abs(v) for v in self.shapley_values.values())

        for sample, value in self.shapley_values.items():
            # 2. Map the value from [-max_abs, max_abs] to [0.0, 1.0] for the colormap
            if max_abs == 0:
                norm_value = 0.5  # Neutral grey if all values are somehow 0
            else:
                norm_value = (value + max_abs) / (2 * max_abs)
        ...
        # 3. Update the colorbar to reflect the new centered scale
        sm = plt.cm.ScalarMappable(
            cmap=plt.cm.coolwarm,
            norm=plt.Normalize(
                vmin=-max_abs,
    ...
    #### End of Updated version of plot_colored_text to center colors on zero and preserve sign #######################################################################
    ...
    def analyze(self, 
        ...
        Args:
            prompt: The user-facing text to analyze (applicant profile only).
                    TokenSHAP splits and masks tokens exclusively from this string.
            system_prompt: Optional frozen instructions passed to every model call
                        as a system message. Never split or masked.
            sampling_ratio: Ratio of combinations to sample (0-1).
            max_combinations: Maximum number of combinations to generate.
            print_highlight_text: Whether to print highlighted text after analysis.
        ...
        # Store system_prompt so _prepare_combination_args can access it
        self._current_system_prompt = system_prompt

        # Get baseline — passes system_prompt through kwargs into _prepare_generate_args
        self.baseline_text = self._calculate_baseline(prompt, system_prompt=system_prompt)
