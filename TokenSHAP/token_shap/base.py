...
class OpenAIModel(ModelBase):
    ...
    def generate(self, prompt: str, system_prompt: Optional[str] = None, image_path: Optional[Union[str, BytesIO]] = None) -> str:
        """Generates text based on a prompt with optional system prompt and vision support.
        
        Args:
            prompt: The user message content.
            system_prompt: Optional system message prepended before the user message.
                        Use this to pass frozen instructions that TokenSHAP should
                        never perturb (formatting constraints, feature reference, etc.)
...
class BaseSHAP(ABC):
    ...
    def _calculate_shapley_values(self, df: pd.DataFrame, content: Any) -> Dict[str, float]:
        ...
        ################## Custom normalization to preserve sign and scale to [-1, 1] #########################################################################
        def normalize_shapley_values(values: Dict[str, float], power: float = 1) -> Dict[str, float]:
            """
            Normalizes Shapley values to the range [-1, 1] while preserving the sign.
            Negative values remain negative, and 0 remains 0.
            """
            powered_values = {}
            for k, v in values.items():
                # Preserve the sign while applying the power transformation
                sign = 1 if v >= 0 else -1
                powered_values[k] = sign * (abs(v) ** power)
                
            # Find the maximum absolute value across all tokens
            max_abs = max(abs(v) for v in powered_values.values())
            
            # Handle the edge case where all values are exactly 0
            if max_abs == 0:
                return {k: 0.0 for k in powered_values}
                
            # Scale values to be between -1.0 and 1.0
            return {k: v / max_abs for k, v in powered_values.items()}
        ################## End #################################################################################################################################
