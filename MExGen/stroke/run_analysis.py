...
from prompts import build_stroke_prompt

# ---------------------------------------------------------------------------
# CLI arguments
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Run MExGen analysis for Stroke")
...
# Drop stroke column if it still exists (fail safe)
if "stroke" in df_final_readable.columns:
    df_final_readable = df_final_readable.drop(columns=["stroke"])
...
for i, idx in enumerate(indices, start=1):
    ...
    patient_profile = row.to_dict()

    system_prompt, prompt, unit_types, ind_segment = build_stroke_prompt(patient_profile)
